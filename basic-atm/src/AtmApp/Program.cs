    ...
    try
    {
        ...
        if (operation[0] == '1')
        {
            ...
            while (!loginStatus)
            {
                ...
                var idCard = Console.ReadLine();
                Console.Write("Pin: ");
                var pin = Console.ReadLine();
                var card = new Card(idCard, pin);
                (loginStatus, var attempts) = atm.Login(card);
            }
            ...
            while (loginStatus)
            {
                ...
                PerformAtmOperation(selectedOperation, atm);
            }
...
void PerformAtmOperation(int opSelected, Atm system)
{
    ...
    else if (opSelected == 2)
    {
        ...
        atm.Deposit(depositAmountConverted);
        Console.WriteLine($"Amount after deposit {atm.CheckBalance()}");
    }
